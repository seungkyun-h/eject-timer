// Plays flat green-screen behavior clips (idle/walk/react) with a real-time chroma key
// (transparent) and a short eased crossfade between behaviors. Three.js full-screen quad +
// fragment shader. The source clips are generated on a FLAT uniform chroma green with a static
// camera and centered full body, so a plain green key + crop is all that's needed. Requires three.
(function (global) {
  ...
  function create(canvas, videos, opts) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        mapB: { value: first },
        blend: { value: 1 },
        crop: { value: new THREE.Vector4(opts.cropX ?? 0.085, opts.cropY ?? 0.21, opts.cropW ?? 0.87, opts.cropH ?? 0.56) },
        thr: { value: opts.thr ?? 0.42 },
        soft: { value: opts.soft ?? 0.07 },
      },
      ...
      fragmentShader: [
        ...
        '  float gf = c.g / (c.r + c.g + c.b + 0.001);',   // green fraction: robust to lighting
        '  float a = 1.0 - smoothstep(thr, thr + soft, gf);',
        '  c.g = min(c.g, max(c.r, c.b) + 0.02);',          // despill green fringe
        '  return vec4(c.rgb, a);',
        '}',
        'void main(){',
